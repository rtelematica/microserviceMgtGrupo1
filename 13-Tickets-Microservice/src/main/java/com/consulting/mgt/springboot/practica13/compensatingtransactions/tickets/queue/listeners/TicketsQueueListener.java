package com.consulting.mgt.springboot.practica13.compensatingtransactions.tickets.queue.listeners;
...
import com.consulting.mgt.springboot.practica13.compensatingtransactions.checkout.queue.producers.CheckoutMicroserviceQueueProducer;
...
import com.consulting.mgt.springboot.practica13.compensatingtransactions.tickets.queue.event.TicketReservationEvent;
...
public class TicketsQueueListener {
	...
	public void ticketReservation(
			@Payload TicketReservationEvent reservationEvent,
		...
		// Implementa
		log.info("---");
		log.info("[Tickets Microservice] applying "
				+ "'ticket reservation' event for ticket order Id {}.",
				reservationEvent.getTicketOrderId());

		if (!successTicketReservation()) {
			log.info(
					"[Tickets Microservice] 'ticket reservation' "
					+ "for ticket order Id {} process failed nothing to compensate.",
					reservationEvent.getTicketOrderId());
			log.info("---");
		} else {
			log.info("[Tickets Microservice] 'ticket reservation' "
					+ "for ticket order Id {} success!.",
					reservationEvent.getTicketOrderId());
			log.info("---");

			checkoutMicroserviceQueueProducer.reservationCheckoutWithdrawal(
					reservationEvent.getUser(), reservationEvent.getTicketOrderId(),
					reservationEvent.getTotal(), "1234-5678-3456-7890");
		}
	}
	...
	public void ticketReservationCompenstation(
			@Payload CancelTicketReservationEvent reservationEvent,
			@Headers MessageHeaders headers,
			Message message,
			Session session) {
		...
		log.info("---");
		log.info(
				"[Tickets Microservice] applying compensation for "
				+ "'cancel ticket reservation compensation' event "
				+ "for ticket order Id {}.",
				reservationEvent.getTicketOrderId());

		if (!successTicketReservationCompensation()) {
			log.info(
					"[Tickets Microservice] compensation for "
					+ "'cancel ticket reservation compensation' event "
					+ "for ticket order Id {} process failed, "
					+ "save event and compensate as soon as possible.",
					reservationEvent.getTicketOrderId());
			log.info("---");
		} else {
			log.info(
